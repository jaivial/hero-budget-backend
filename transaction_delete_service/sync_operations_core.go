...
// Auto-detects and fixes missing operation types for transaction delete service
func updateSyncOperationsSchema() error {
	log.Printf("🔧 Checking and updating sync_operations schema for transaction delete service...")
	
	// Test if we can insert an operation with transaction-specific operation types
	...
	_, err := db.Exec(testQuery,
		"test_user",
		testOperationId,
		"delete",  // Test standard operation type
		"transactions",
		"test_id",
		string(testData),
		string(deviceIds),
...
func addSyncOperationEnhanced(userID, providedOperationID, action, tableName, recordID string, data interface{}, deviceID string, clientTimestamp int64) error {
	...
	result, err := db.Exec(
		...
		userID,
		operationID,
		action,            // operation_type (delete for transaction delete service)
		tableName,         // entity_type (transactions)
		recordID,          // entity_id (transaction_id)
