...
func init() {
	...
	// Get the current working directory to establish database path
	cwd, err := os.Getwd()
	if err != nil {
...
func main() {
	...
	http.HandleFunc("/expense/add", corsMiddleware(handleAddExpense))
	http.HandleFunc("/expense/update", corsMiddleware(handleUpdateExpense))
	http.HandleFunc("/expense/delete", corsMiddleware(handleDeleteExpense))
	http.HandleFunc("/expense/fetch", corsMiddleware(handleFetchExpenses))
	http.HandleFunc("/expense/analytics/daily", corsMiddleware(handleDailyAnalytics))
	http.HandleFunc("/expense/analytics/weekly", corsMiddleware(handleWeeklyAnalytics))
	http.HandleFunc("/expense/analytics/monthly", corsMiddleware(handleMonthlyAnalytics))
	http.HandleFunc("/expense/analytics/quarterly", corsMiddleware(handleQuarterlyAnalytics))
	http.HandleFunc("/expense/analytics/semiannual", corsMiddleware(handleSemiannualAnalytics))
	http.HandleFunc("/expense/analytics/annual", corsMiddleware(handleAnnualAnalytics))
